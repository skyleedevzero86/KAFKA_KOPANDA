...
class Topic(
    ...
    private val _partitions: MutableList<Partition> = mutableListOf(),
    ...
    fun removePartition(partitionNumber: Int) {
        _partitions.removeIf { it.partitionNumber.value == partitionNumber }
        updatedAt = LocalDateTime.now()
    }

    fun getPartition(partitionNumber: Int): Partition? {
        return _partitions.find { it.partitionNumber.value == partitionNumber }
    }

    fun getPartitionCount(): Int = _partitions.size

    fun getTotalMessageCount(): Long = _partitions.sumOf { it.messageCount }

    fun isHealthy(): Boolean = _partitions.all { it.isHealthy() }

    fun getUnhealthyPartitions(): List<Partition> = _partitions.filter { !it.isHealthy() }
